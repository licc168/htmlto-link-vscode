...
exports.normalizeUiLocale = normalizeUiLocale;
exports.isUiLocale = isUiLocale;
exports.getPreferredUiLocale = getPreferredUiLocale;
exports.setPreferredUiLocale = setPreferredUiLocale;
...
exports.messages = {
    'zh-CN': {
        panel: {
            panelTitle: '静态网站一键发布',
            heroTitle: '静态网站一键发布',
            heroDescription: '把本地静态 HTML 项目发布成可访问链接。支持游客 24 小时临时部署，也支持使用 Token 进行长期可管理部署。',
            ...
            previousShareGuestUnavailable: '检测到旧的游客部署记录，但缺少 updateToken 或链接已不适合继续更新，本次将默认创建新链接：{url}',
            previousShareIdentityUnavailable: '检测到历史部署记录，但当前身份不能直接更新这个链接：{url}',
            ...
            customTokenHint: '没有 Token 也可以先游客部署；如果需要长期保存链接，点击上方按钮查看获取方式。',
            saveTokenLabel: '保存 Token，后续部署可直接复用',
            guestNote: '当前为游客部署模式，发布成功后链接仅保留 24 小时。若需要长期保存和版本更新，建议使用 Token。',
            deploy: '开始部署',
            deploying: '部署中...',
            ...
            guestConfirmText: '当前将以游客身份发布，适合快速试用，但链接只会保留 24 小时。',
            guestConfirmList: [
            ...
            clearTokenText: '这会删除当前编辑器里保存的 HTML to Link Token，之后将不能直接复用登录身份部署。',
            clearTokenList: [
                '清除后不会影响已经生成的链接。',
            ...
            tokenHelpText: '使用 Token 部署后，链接可长期保留，也方便后续继续更新版本。',
            tokenHelpSteps: [
                {
                    title: '先注册账号',
        ...
        sidebar: {
            ...
            headerDescription: '左侧固定入口。适合随时打开完整发布面板、选择文件夹、配置 Token，以及快速回到最近发布链接。',
            ...
            recommendedGuestDesc: '未检测到已保存 Token，适合临时分享。需要长期更新链接时可先配置 Token。',
            openPanel: '打开完整发布面板',
        ...
        quickPublish: {
            ...
            noTarget: '未找到可发布的 HTML。请打开一个 .html 文件，或在含 index.html 的文件夹中使用。',
            success: '发布成功：{url}',
    ...
    en: {
        panel: {
            ...
            heroDescription: 'Publish local static HTML projects to a public URL. Supports 24-hour guest sharing and token-based long-term deployments.',
            ...
            entryManualHint: 'No HTML file was detected automatically. Enter the entry file path relative to the folder.',
            previousShareReusable: 'A previous deployment was found and can be updated: {url}',
            previousShareGuestUnavailable: 'A previous guest deployment was found, but the update token is missing or the link can no longer be updated. A new link will be created: {url}',
            previousShareIdentityUnavailable: 'A previous deployment was found, but the current identity cannot update this link directly: {url}',
            ...
            customTokenHint: 'You can start with guest publishing first. If you need persistent links, use the button above to learn how to get a token.',
            saveTokenLabel: 'Save this token so future publishes can reuse it',
            guestNote: 'You are using guest publish mode. The published link lasts for 24 hours only. Use a token if you need long-term links and version updates.',
            ...
            reuseExistingDeployment: 'If a previous deployment is found, create the new version on the existing link',
            currentFolderHint: 'Current folder: {folderPath}',
            deployHintWithoutFolder: 'After you choose a folder, the extension will try to detect the entry file automatically.',
            ...
            guestConfirmText: 'This will publish as a guest. It is great for quick sharing, but the link only lasts 24 hours.',
            guestConfirmList: [
            ...
            clearTokenText: 'This removes the HTML to Link token saved in the current editor, so it can no longer be reused directly.',
            clearTokenList: [
            ...
            tokenHelpText: 'Publishing with a token keeps links available long-term and makes later updates easier.',
            tokenHelpSteps: [
                {
            ...
            toastReuseUnavailable: 'The previous deployment record cannot be reused right now, so a new link will be created.',
            ...
            errorSavedTokenMissing: 'No saved token is available. Enter a new token or switch to guest publish mode.',
        ...
        sidebar: {
            ...
            headerDescription: 'A fixed sidebar entry for opening the full publish panel, selecting a folder, configuring a token, and jumping back to the latest public link.',
            ...
            recommendedGuestDesc: 'No saved token was detected. Guest publish works well for temporary sharing.',
        ...
        quickPublish: {
            ...
            noTarget: 'No publishable HTML found. Open a .html file, or use a folder that contains index.html.',
...
async function getPreferredUiLocale(context) {
    const stored = context.globalState.get(UI_LOCALE_KEY);
    if (stored && isUiLocale(stored)) {
        return stored;
    }
    return normalizeUiLocale(vscode.env.language);
}
async function setPreferredUiLocale(context, locale) {
